...
This buffer then then set up to be used for a screenshot."
  (let ((hl (bound-and-true-p hl-line-mode)))
    ...
    (prog1
        (with-current-buffer
            (if (>= emacs-major-version 28) ; Fourth optional arg added in Emacs 28
                (make-indirect-buffer (current-buffer) " *screenshot-clone" t t)
              (make-indirect-buffer (current-buffer) " *screenshot-clone" t))
